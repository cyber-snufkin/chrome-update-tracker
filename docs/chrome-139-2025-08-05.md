...
| **scope_extensions** | Web App Manifest の新フィールド。複数オリジン（サブドメインや別ドメイン）を単一 PWA として統合。 | Web アプリ |
...
- サンプル：
  ...
  @function --theme-color(--variant) {
    result: if(style(--variant: primary), oklch(60% 0.2 240),
               if(style(--variant: secondary), oklch(70% 0.15 120),
                  oklch(50% 0.1 0)));
  }
...
- サンプル：
  ```javascript
  // オンデバイス音声認識の利用可能性確認と使用
  const canUseLocal = await navigator.speechRecognition
    ?.canUseLocalRecognition?.('ja-JP');

  if (canUseLocal) {
    const recognition = new webkitSpeechRecognition();
    recognition.lang = 'ja-JP';
    recognition.localService = true; // ローカル処理を指定
    ...
    console.log('オンデバイス音声認識は利用できません');
    // クラウド版にフォールバック
  }
  ```
