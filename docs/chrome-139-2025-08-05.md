...
- サンプル：
  ```javascript
  // オンデバイス音声認識の利用可能性確認
  // ※この API は仕様策定中で変更の可能性があります
  const availabilityStatus = await SpeechRecognition.availability({
    langs: ['ja-JP'],
    processLocally: true
  });

  // 戻り値: 'available' | 'unavailable' | 'downloadable' | 'downloading'
...
- **Web API**: オンデバイス音声認識は利用可能性確認（`SpeechRecognition.availability()`）を必ず実装。フォールバックとしてクラウド版を用意すること
