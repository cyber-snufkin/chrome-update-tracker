...
> 本Glossaryは Chrome 134 で新登場または重要化した技術用語の一覧です。

| 用語 | 概要 | 関連領域 |
|------|------|-----------|
| **`<dialog closedby>`** | `closedby="none\|closerequest\|any"` でライトディスミス挙動を制御。`any` で外側クリック・ESC で閉じる。 | HTML / UX |
| **カスタマイズ可能な `<select>`** | `appearance: base-select` で opt-in し、`<option>` 内に画像など追加要素を配置可能。リッチな選択 UI を実現。 | HTML / CSS |
| **SelectParserRelaxation** | `<select>` 内の追加タグを許容する一時移行ポリシー。Chrome 141 で終了予定。 | HTML / ポリシー |
| **Shared Storage × Web Locks** | Shared Storage の worklet 内で `navigator.locks.request` 利用可能。`withLock` / `batchUpdate()` で原子的更新。 | Web API / プライバシー |
| **`imageSmoothingQuality`** | Canvas 2D で画像拡縮時の品質を `'low'\|'medium'\|'high'` で指定。 | Canvas / パフォーマンス |
| **PWA Link Capture** | manifest の `capture_links: "new-client"` でインストール済み PWA へのリンクキャプチャを制御。 | PWA / UX |
| **`dynamic-range-limit`** | HDR ディスプレイの最大輝度を制御。`@media (dynamic-range: high)` 内で白飛び回避。 | CSS / HDR |
| **WebGPU subgroups** | SIMD レベルの集団演算。`adapter.features.has('subgroups')` で検出し ML/映像処理を高速化。 | WebGPU / パフォーマンス |
| **`console.timeStamp()`** | パフォーマンストレースに軽量マーカーを注入。DevTools の Performance タイムラインに独自ラベルを追加。 | DevTools / パフォーマンス |
| **Baseline** | 横断的な実装・安定化が進んだ機能群の合意点。導入判断を簡素化。 | 互換性 |
| **Origin Trial (OT)** | 期限付き実験。トークン配布で実使用下の評価を可能に。 | 実験機能 |
