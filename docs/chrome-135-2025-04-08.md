...
> 本Glossaryは **docs/chrome-135-2025-04-08.md** の「全体サマリ」の直後に挿入予定の内容です（PR前の準備版）。

| 用語 | 概要 | 関連領域 |
|------|------|-----------|
| **CSS `@scope`** | Shadow DOM なしでスタイルスコープを実現。`@scope (.card) { h2 { ... } }` で特定要素配下のみにスタイルを限定。 | CSS |
| **Display-P3 カラー** | `color(display-p3 r g b)` で広色域・高彩度 UI を実現。P3 対応モニタで鮮やかな色表現。 | CSS / 色 |
| **`<dialog>` returnValue** | `<form method=dialog>` のボタン `value` を `dialog.returnValue` で取得。閉じ方に応じた分岐処理が簡潔に。 | HTML / フォーム |
| **RegExp Unicode sets 最適化** | `\p{…}` / `\q{…}` などの Unicode プロパティエスケープの性能向上。多言語テキスト処理の高速化。 | JavaScript / 正規表現 |
| **CookieStore API** | `CookieStoreObserver` で Cookie のリアルタイム監視。変更検知によるリアクティブな状態管理。 | Web API / ストレージ |
| **Gemini Nano（DevTools統合）** | Chrome 内蔵 LLM が DevTools に統合され、要素選択・コード生成・パフォーマンス改善を自然言語でサジェスト。 | AI / DevTools |
| **DevTools Recorder** | Playwright / Puppeteer 形式でのコード生成対応。UI 操作を自動でスクリプト化。 | DevTools / テスト |
| **Baseline 2025** | `@scope`, `:has()`, `color-mix()` などが新たに横断互換に。設計時の採用判断が容易に。 | 互換性 |
